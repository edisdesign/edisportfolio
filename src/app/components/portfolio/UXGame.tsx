...
export const UXGame = ({ language }: UXGameProps) => {
  ...
  // Web Audio API for zero-latency synthesized UI micro-sounds
  const playUXSound = (type: 'grab' | 'snap' | 'success') => {
    try {
      ...
      // Silently fail if audio context is blocked (e.g. before user interaction)
    }
  };
  ...
  useEffect(() => {
    if (Object.values(matchedShapes).every(Boolean) && !hasWon) {
        setHasWon(true);
        playUXSound('success');
    }
  }, [matchedShapes]);
